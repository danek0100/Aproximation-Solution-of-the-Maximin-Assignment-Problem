#ifndef RECTANGLE_UNION_CONTOUR_FINDER_H
#define RECTANGLE_UNION_CONTOUR_FINDER_H
...
class RectangleUnionContourFinder
{
private:
	...
	static std::unique_ptr < std::pair < std::vector<VerticalEdge> , std::vector<HorizontalEdge> > > contourOfUnionOfRectangles(const std::vector<Rectangle>& rectangles )
	{
		...
		for ( size_t i = 0; i < X.size(); ++i )
		{
			if ( X[i].position == 0 )
			{
				st.root->contr( X[i].beginY, X[i].endY, stack );
				accumulateVerticalEdgesAndEndpoints( A, verticalEdges, X[i].x, stack );
				st.root->insert( X[i].beginY, X[i].endY );
			}
			else
			{
				st.root->remove( X[i].beginY, X[i].endY );
				st.root->contr( X[i].beginY, X[i].endY, stack );
				accumulateVerticalEdgesAndEndpoints( A, verticalEdges, X[i].x, stack );
			}
			stack.clear();
